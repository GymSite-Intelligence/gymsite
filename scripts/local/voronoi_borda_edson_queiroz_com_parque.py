"""Re-run Cocó→Edson Queiroz border capture with local Maps rivals."""
...
def main() -> None:
    ...
    neigh = by[normalizar_bairro(VIZ)].geometry
    ring = _ring(neigh)
    lat, lng = _pin(coco, neigh)
    ...
    viz_maps = concorrentes_from_searchapi_local_results(
        json.loads(
            (
                ROOT / "tools/fixtures/searchapi_maps_edson_queiroz_fortaleza.json"
            ).read_text(encoding="utf-8")
        )
    )
    seen = set()
    maps = []
    for m in coco_maps + viz_maps:
    ...
    in_viz = [m for m in maps if point_in_ring(float(m["lng"]), float(m["lat"]), ring)]
    print("pin", lat, lng)
    print("rivals_total", len(maps), "inside_edson_poly", len(in_viz))
    for m in in_viz:
    ...
    kw = dict(
        ...
        idade_min=25,
        idade_max=40,
        pin_fonte="borda_coco_capta_edson_queiroz",
    )
    ...
    out = {
        "cenario": "pin_borda_coco_demanda_edson_queiroz",
        "pin": {"lat": lat, "lng": lng},
        "pop_vizinho": pop,
        "estoque_form_vizinho": est,
        "pool_form_vizinho": pool_ref,
        "rivals_coco_only": len(coco_maps),
        "rivals_viz_added": len(viz_maps),
        "rivals_merged": len(maps),
        "rivals_inside_viz_poly": len(in_viz),
        "smoke_so_coco_maps": {
        ...
        "comparativo_tauape_com_parque": {
            "pool_voronoi": 31,
            "nota": "borda Cocó→Tauape com 6 Maps Tauape",
        },
    }
    path = ROOT / ".superpowers/sdd/voronoi-borda-edson-queiroz-com-parque.json"
