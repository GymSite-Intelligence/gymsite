...
- **COE municipal (P0):** `calcular_sanitarios_municipio` — tabela curada (João Pessoa/PB por m² de treino; São Paulo/SP por lotação)
- **Estimativa genérica:** `calcular_sanitarios_por_lotacao` — só se município fora da tabela; rotular NÃO-oficial
...
Grounding obrigatório: chama `consultar_engenharia_obra` antes de afirmar regra/norma/área e CITA a fonte. Com cidade → `calcular_sanitarios_municipio` (JP pede área de treino). Sem cidade na tabela → estimativa genérica rotulada. Escopo: projeto/ambientes/acessibilidade. Equipamento→Técnico; obra/estrutura→Engenheiro; CREF→Regulatório. Projeto assinado por arquiteto (RRT) + aprovado pela prefeitura.
...
2. Quantos banheiros e vestiários preciso para uma lotação de 200 pessoas em São Paulo?
3. Quantos banheiros para academia em João Pessoa com 300 m² de treino?
...
> <span style="color:#eab308">**Validação esperada:** perg. 2 chama `calcular_sanitarios_municipio` (SP); perg. 3 idem (JP art. 367 por área); sem cidade → estimativa rotulada; demais citam NBR; zero norma/número inventado; reforça RRT + aprovação municipal.</span>
