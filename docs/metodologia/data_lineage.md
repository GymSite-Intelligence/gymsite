...
| **Wikipedia pt / Wikidata** (município) | runtime MediaWiki + cache `data/wiki_municipios/{cidade}_{UF}.json` (TTL 30d) · `tools/wikipedia_municipio.py` | TTL 30d / `WIKI_MUNICIPIO_FORCE_REFRESH` | A0 `carregar_wikipedia_municipio` → `contexto_local_wiki` (**display_only**; nunca parque/score/MRLR) |
...
| **Portais imobiliários** (ZAP/Viva/OLX) | removido (obsoleto) | legado | Removido — aluguel de viabilidade é MRLR Tier 0 (tools/aluguel_mrlr.py); fallback é benchmark setorial/BCB |
