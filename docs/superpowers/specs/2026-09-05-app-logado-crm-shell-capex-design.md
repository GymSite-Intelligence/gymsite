...
**Date:** 2026-09-05  
**Status:** approved — implementation plan `docs/superpowers/plans/2026-09-05-app-logado-crm-shell-capex.md`  
...
- Displayed CAPEX uses total **minus** equipment kit (and coupled equipment freight if currently inside CAPEX)
- Metrics that are **pure functions of CAPEX** on the client (e.g. payback if derived in viewer) recompute from adjusted CAPEX
- **TIR / ratios already stored** by pipeline with kit included: either (1) recompute in viewer if inputs exist, or (2) if not safely recomputable without A4 — show adjusted CAPEX and keep stored TIR only with explicit follow-up ticket to stop emitting kit in A4 (do not silently show inconsistent TIR as “sem kit”)
- Consórcio never enters totals
- Pipeline/A4 kits remain in repo this wave; presentation layer enforces hide + CAPEX subtract first
