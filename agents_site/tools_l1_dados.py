"""L1 deterministic tools — Maps, IBGE, CNPJ, MRLR, formulas, layout."""
...
logger = logging.getLogger("gymsite.agents_site.tools_l1")
...
async def buscar_pontos_comerciais(cidade: str, bairro: str, uf: str,
    ...
    """Anúncios individuais no bairro com aluguel de decisão MRLR."""
    import asyncio
    ...
    if result.get("aviso"):
        result["aviso_usuario"] = result["aviso"]
    return result
...
def buscar_concorrentes(
    ...
    raio_metros: int = 1000,
) -> dict:
    """Conta e lista academias concorrentes (SearchAPI / Maps ao vivo).
    Formato 1: query tipada + exclude pós-fetch — NUNCA estime a quantidade de cabeça.
    Inclusão: dist ≤ raio (default 1000 m canônico) do centróide + gate tipo/status.
    Bairro na query = recall only. Link Maps bruto ≠ N filtrado.
    ...
    Args:
        ...
        bairro: bairro de referência (ex.: "Cocó") — hint de query + geocode.
        uf: sigla do estado, opcional (ex.: "CE").
        tipo_negocio: "academia" | "crossfit" | "studio_pilates" | "studio_funcional".
        raio_metros: raio de inclusão em metros (300 a 5000; padrão 1000 canônico).

    Returns:
        dict Formato 1: total, nivel_saturacao, concorrentes (lista completa até 40),
        query, exclude_aplicado, maps_smoke_url (URL = query bruta), raio_metros.
    """
    from tools.competitor_tools import (
        RAIO_CONCORRENCIA_CANONICO_M,
    ...
    try:
        raio = max(300, min(5000, _as_int(raio_metros, RAIO_CONCORRENCIA_CANONICO_M)))
        ...
        return {
            ...
            "raio_metros": RAIO_CONCORRENCIA_CANONICO_M,
            "maps_smoke_aviso": "lista_bruta_maps_diferente_do_total_filtrado",
        }
...
def analisar_reviews_e_dores(
    ...
    try:
        res = buscar_academias(bairro, cidade, 1000, uf or "", tipo_negocio)
    except Exception as e:  # noqa: BLE001
    ...
    def _distancia_metros(c: dict) -> float:
        dm = c.get("distancia_m")
        if dm is not None:
            try:
                return float(dm)
            except (TypeError, ValueError):
                pass
        try:
            return float(c.get("distancia_km") or 99) * 1000.0
        except (TypeError, ValueError):
            return 99_000.0

    brutos.sort(key=_distancia_metros)
...
def gerar_planta_layout_zonas(
    area_m2: float,
    comprimento_m: float = 0.0,
    largura_m: float = 0.0,
    preset: str = "musculacao_entrada_fundo",
) -> dict:
    """Gera planta baixa SVG (zonas + fluxo) para sala de musculação — ANTEPROJETO, não RRT.

    Use SEMPRE que o usuário pedir layout, planta, fluxo de zonas, croqui ou distribuição
    espacial da musculação. Não desenhe ASCII no lugar — chame esta tool e descreva as zonas
    retornadas. Para normas de circulação/acessibilidade, combine com `consultar_engenharia_obra`.

    Área de cálculo: mediana da faixa de porte (igual A4), não a área bruta.
    Ex.: 1400 m² → faixa M 800–1500 → envelope sobre 1150 m² (proporção 4:3 se sem L×C).
    Declare no texto a faixa e a mediana (`dimensoes_m.area_calculo_m2`).

    Args:
        area_m2: área informada pelo usuário (m²) — entra no gate de faixa/mediana.
        comprimento_m: opcional — eixo entrada→fundo (m). Se 0, deriva da mediana (4:3).
        largura_m: opcional — eixo lateral (m). Informe L×C se o usuário souber.
        preset: layout de zonas. Hoje só `musculacao_entrada_fundo` (entrada → racks no fundo).

    Returns:
        dict com `zonas`, `dimensoes_m` (inclui area_informada/area_calculo/faixa), `fluxo`, `svg`.
    """
    from tools.floor_plan_layout import render_layout_svg
    ...
    try:
        ...
        logger.exception("gerar_planta_layout_zonas falhou")
        return {"erro": f"{type(e).__name__}: {e}", "zonas": [], "svg": ""}
