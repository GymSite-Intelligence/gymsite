...
export function ConsultorChat({
  ...
  const submit = (e: React.FormEvent) => {
    ...
    onSend(txt, apiIdFromUi(focado.id))
    setInput('')
  }
    ...
    <>
      ...
      <div className="flex flex-col gap-1 px-2 pb-2">
        {ESPECIALISTAS.map((e) => {
          ...
          return (
            ...
            >
              <img src={e.img} alt="" className="h-9 w-9 shrink-0 object-contain" />
              <div className="min-w-0 flex-1">
                <span className="block truncate text-xs font-semibold">{e.nome}</span>
      ...
      <div className="flex items-center justify-between px-3 py-2.5">
        ...
        <Button
          variant="ghost"
          size="sm"
          className="h-7 gap-1 px-2 text-[11px] text-lime hover:text-lime"
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex shrink-0 items-center gap-2 border-b border-border bg-card/80 px-2 py-1.5 sm:px-3">
          <button
            ...
            aria-controls={isLg ? 'sidebar-consultor-especialistas' : 'sidebar-consultor-especialistas-sheet'}
          >
            <PanelLeft className="h-4 w-4 shrink-0 text-lime" aria-hidden />
            <span className="max-w-28 truncate sm:max-w-none">{especialistaAtivo.nome}</span>
          </button>
