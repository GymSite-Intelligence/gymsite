...
def extrair_genero_do_state(state: dict, limiar: float | None = None) -> dict:
    ...
    """
    lim = float(limiar if limiar is not None else limiar_genero_pp())
    demo_raw = state.get("demografia_bairro")
    demo: dict = demo_raw if isinstance(demo_raw, dict) else {}
    if not demo:
        oc_raw = state.get("output_consolidado")
        oc: dict = oc_raw if isinstance(oc_raw, dict) else {}
        demo_fb = oc.get("demografia_bairro")
        demo = demo_fb if isinstance(demo_fb, dict) else {}
