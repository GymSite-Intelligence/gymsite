...
class NvidiaOllamaFallbackLlm:
    ...
    def __init__(
        ...
        *,
        ...
        self._primary = LiteLlm(model=model, api_key=api_key)
    ...
    async def generate_content_async(self, llm_request: Any, stream: bool = False) -> Any:
        yielded = False
        try:
            async for chunk in self._primary.generate_content_async(llm_request, stream=stream):
                yielded = True
                yield chunk
            return
        ...
        try:
            if hasattr(llm_request, "model"):
                llm_request.model = self._ollama_fallback_model
            ollama = LiteLlm(
                model=self._ollama_fallback_model,
